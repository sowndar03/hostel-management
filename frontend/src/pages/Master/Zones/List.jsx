...
const List = () => {
  ...
  const getAllBuildings = async () => {
    try {
      const res = await api.get(`${api_url}/master/zone/list`);
  ...
  const handleSearch = async (data) => {
    try {
      const result = await api.post(`${api_url}/master/zone/searchValues`, data);
  ...
  const handleStatusClick = async (id, status) => {
    ...
    if (status === 1) {
      text = "Do you want to Inactivate the Zone?";
      button = "Yes, Inactivate!";
    } else {
      text = "Do you want to Activate the Zone?";
      button = "Yes, Activate!";
    }
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.isConfirmed) {
        try {
          const result = await api.post(`${api_url}/master/zone/statusChange`, {
            id,
            status,
          });
          setList((prevList) =>
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await api.post(`${api_url}/master/zone/delete`, { id });
          setList((prevList) => prevList.filter((item) => item._id !== id));
          Swal.fire(
            "Updated!",
            "Zone has been Deleted Successfully",
            "success"
          );
        } catch (err) {
          Swal.fire("Oops...", "Something went wrong!", "error");
  ...
  const handleView = (id) => {
    navigate(`/master/zone/view/${id}`);
  };
  const handleEdit = (id) => {
    navigate(`/master/zone/edit/${id}`);
  };
    ...
    <div className='min-h-screen bg-white dark:bg-[#101828] p-6'>
      <div className="border-b pb-3 mb-4">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-bold text-gray-700 dark:text-white">Zone List</h2>
          <div>
            <button
              type="button"
            ...
            <button
              ...
              onClick={() => navigate("/master/zone/add")}
            >
              Add
            </button>
          </div>
