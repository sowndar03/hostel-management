...
const Add = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const res = await api.post(`${api_url}/master/zone/add`, data);
  ...
  const getMap = async (location_id) => {
    try {
      const res = await api.get(`${api_url}/master/location/view/${location_id}`);
      setAreaValue(res.data.data.location_name);
      console.log(res);
    } catch (err) {
      console.log(err);
    }
    ...
    <div className='min-h-screen bg-white dark:bg-[#101828] p-6'>
      <form onSubmit={handleSubmit(onSubmit)} className='rounded shadow-lg p-6'>
        <div className="flex justify-between items-center border-b pb-3 mb-4">
          <h2 className="text-lg font-bold text-gray-700 dark:text-white">Zone</h2>
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
                ...
                validate: async (value) => {
                  ...
                  try {
                    const res = await api.post(
                      `${api_url}/master/zone/uniqueCheck`,
                      { hostel_id: value, location_id }
                    );
                    ...
                    return res.data.message || "This hostel already exists";
                  } catch (err) {
          ...
          <div className="">
            <label htmlFor="lattitude" className='block mb-2 text-gray-700 dark:text-white font-semibold'>Lattitude <span className='text-red-500'>*</span></label>
            <input
              type="text"
            ...
            {
              errors.zone_lat && <p className="text-red-500 text-sm mt-1 font-bold">
                {errors.zone_lat.message}
              </p>
            }
          </div>

          <div className="">
            <label htmlFor="longitude" className='block mb-2 text-gray-700 dark:text-white font-semibold'>Longitude <span className='text-red-500'>*</span></label>
            <input
              readOnly
              type="text"
